#!/usr/bin/env python3
"""Build arb_engine/data/<sport>_teams.json from ESPN's team list, then learn the venue codes that
differ from ESPN's abbreviations: Kalshi ticker codes (from open game markets, resolved through
their titles) and Robinhood short names (from the cached catalogue). Re-run any time; manual
aliases survive.

    ARB_HTTP_TRANSPORT=curl python3 scripts/build_teams.py --sport ncaaf|nba|nhl
"""
...
SPORTS = {
    "ncaaf": {"espn": "football/college-football", "kalshi": "KXNCAAFGAME", "robinhood": "college-football", "prefixes": ("NX.F.OPT.CFB", "KXNCAAFGAME", "NCAAFGAME"), "limit": 1000},
    "nba": {"espn": "basketball/nba", "kalshi": "KXNBAGAME", "robinhood": "nba", "prefixes": ("KXNBAGAME", "NBAGAME", "NX.F.OPT.NBA"), "limit": 100},
    "nhl": {"espn": "hockey/nhl", "kalshi": "KXNHLGAME", "robinhood": "nhl", "prefixes": ("KXNHLGAME", "NHLGAME", "NX.F.OPT.NHL"), "limit": 100},
}
MANUAL_ALIASES_BY_SPORT = {
    "nba": {"NY": ["New York", "NYK", "Knicks"], "GS": ["Golden State", "GSW", "Warriors"], "SA": ["San Antonio", "SAS", "Spurs"], "NO": ["New Orleans", "NOP", "Pelicans"], "UTAH": ["Utah", "UTA", "Jazz"], "WSH": ["Washington", "WAS", "Wizards"], "PHX": ["Phoenix", "PHO", "Suns"], "BKN": ["Brooklyn", "BRK", "Nets"], "OKC": ["Oklahoma City", "Thunder"], "LAL": ["Los Angeles Lakers", "LA Lakers", "Lakers"], "LAC": ["Los Angeles Clippers", "LA Clippers", "Clippers"]},
    "nhl": {"LA": ["Los Angeles", "LAK", "Kings"], "NJ": ["New Jersey", "NJD", "Devils"], "SJ": ["San Jose", "SJS", "Sharks"], "TB": ["Tampa Bay", "TBL", "Lightning"], "VGK": ["Vegas", "Las Vegas", "Golden Knights"], "UTA": ["Utah", "Utah Mammoth", "Mammoth", "UTAH"], "WSH": ["Washington", "WAS", "Capitals"], "MTL": ["Montreal", "Montréal", "MON", "Canadiens"], "NYR": ["New York Rangers", "NY Rangers", "Rangers"], "NYI": ["New York Islanders", "NY Islanders", "Islanders"], "CBJ": ["Columbus", "Blue Jackets"], "SEA": ["Seattle", "Kraken"]},
}
MANUAL_ALIASES = {  # ncaaf: canonical ESPN abbreviation -> extra spellings seen on venues
...
def main() -> int:
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "arb_engine", "data", f"{args.sport}_teams.json")
    manual = MANUAL_ALIASES if args.sport == "ncaaf" else MANUAL_ALIASES_BY_SPORT.get(args.sport, {})
    extra = EXTRA_TEAMS if args.sport == "ncaaf" else {}
    h = HttpClient(transport=os.environ.get("ARB_HTTP_TRANSPORT"))
    data = h.get(f"https://site.api.espn.com/apis/site/v2/sports/{cfg['espn']}/teams", {"limit": cfg["limit"]}, headers={"Accept": "application/json"})
    ...
    for code, (name, nick, aliases) in extra.items():
        out.setdefault(code, {"name": name, "nick": nick, "espn_id": None, "aliases": list(aliases)})
    for code, extra_aliases in manual.items():
        if code in out:
    ...
    learned = 0
    try:
        k = h.get("https://api.elections.kalshi.com/trade-api/v2/markets", {"series_ticker": cfg["kalshi"], "status": "open", "limit": 1000})
    ...
    cat = os.path.join(os.path.dirname(__file__), "..", "out", "cache", f"robinhood_{cfg['robinhood']}_catalogue.json")
    if os.path.exists(cat):
        ...
        for ev in evs:
            for c in (ev.get("eventContracts") or {}).values():
                if not str(c.get("symbol", "")).startswith(tuple(cfg["prefixes"])):
                    continue
    ...
    print(f"{args.sport}: {len(out)} teams, {learned} venue aliases learned -> {out_path}")
    return 0
