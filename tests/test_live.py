...
class LiveSlateTests(unittest.TestCase):
    ...
    def test_recording_call_sites_and_pregame_anchor(self):
        ...
        # Third identical poll: clock-frozen -> a GATED record, journaled as info, no new alert.
        slate.tick(now + 10)
        slate.tick(now + 20)
        ...
        self.assertEqual(len(store.calls["record_espn_tick"]), 3)
    ...
    def test_cli_plugin_registers_gate_flags_once(self):
        ...
        # Registering again is a no-op for the flags (idempotent under a double plugin load).
        register(sub, {"live": lv, "inplay": ip})
        self.assertEqual(sum(1 for a in lv._actions if "--stale-after" in a.option_strings), 1)
        # The wrapped handler forwards to cli.cmd_live with the settings dict filled in.
