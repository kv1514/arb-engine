...
globalThis.fetch = async (url) => {
  ...
  if (url.startsWith("http://127.0.0.1:8765/inplay")) return res(200, '{"ok":true,"view":{"actions":["wait: Tennessee locks a profit at <= 0.46"],"live":false}}');
  if (url.startsWith("data:application/json,")) return res(200, decodeURIComponent(url.slice("data:application/json,".length)));
  if (url.includes("/prediction-markets/nfl/events/september-20-carolina-vs-atlanta-totals")) return res(200, FIXTURES["rh_totals_page.html"]);
  if (url.includes("/prediction-markets/nfl/events/")) return res(200, FIXTURES["rh_event_page.html"]);
  if (url.includes("/marketdata/event/contract/quotes/v1/")) return res(200, url.includes("2c58") || /NFLTOTAL|CARATL/.test(url) || __totalsMode ? FIXTURES["rh_totals_quotes.json"] : FIXTURES["rh_quotes.json"]);
  let k = /trade-api\/v2\/markets\?event_ticker=([A-Z0-9-]+)/.exec(url);
  if (k) return FIXTURES["kalshi_event_" + k[1] + ".json"] ? res(200, FIXTURES["kalshi_event_" + k[1] + ".json"]) : res(200, '{"markets":[]}');
  k = /gamma-api\.polymarket\.com\/events\?slug=([a-z0-9-]+)$/.exec(url);
  if (k) return FIXTURES["pm_event_" + k[1] + ".json"] ? res(200, FIXTURES["pm_event_" + k[1] + ".json"]) : res(200, "[]");
