const DEFAULTS = { gold: false, contracts: 100, targetMargin: 0, kalshiRounding: "cent", refreshSeconds: 15, venues: { kalshi: true, polymarket: true }, bridge: "auto", positions: "" };
...
$("save").addEventListener("click", () => {
  const s = { gold: $("gold").checked, contracts: Math.max(1, Number($("contracts").value) || 100), targetMargin: Math.max(0, Number($("targetMargin").value) || 0), refreshSeconds: Math.max(5, Number($("refreshSeconds").value) || 15), kalshiRounding: $("kalshiRounding").value, bridge: $("bridge").value, positions: $("positions").value, venues: { kalshi: $("venue_kalshi").checked, polymarket: $("venue_polymarket").checked } };
